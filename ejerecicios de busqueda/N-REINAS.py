from tkinter import *
class reinas():
	def __init__(self,matriz,interfaz,iaux,jaux,primerTiro):
		self.interfaz=interfaz
		self.tablero=matriz
		self.tamanio=len(self.tablero)
		if(not primerTiro):
			print(iaux,jaux)
			print(self.tablero[3][7])
			self.marcar(iaux,jaux)

		self.llenarMatriz()
	def llenarMatriz(self):
		for i in range(0,self.tamanio):
			for j in range(0,self.tamanio):
				if(self.disponible(i,j)):
					self.tablero[i][j]='R'
					self.interfaz.mover(i,j,"R")
					self.marcar(i,j)
	def disponible(self,i,j):
		return self.tablero[i][j]!='x'
	def marcar(self,i,j):
		self.marcarHorizontal(i)
		self.marcarInclinado(i,j)
		self.marcarVertical(j)
	def marcarHorizontal(self,i):
		for n in range(0,self.tamanio):
			if(self.tablero[i][n]!='R'):
				self.tablero[i][n]='x'
	def marcarInclinado(self,i,j):
		auxi=i
		auxj=j
		while(auxi>=0 and auxj>=0):
			if(self.tablero[auxi][auxj]!='R'):
				self.tablero[auxi][auxj]='x'
			auxi-=1
			auxj-=1
		auxi=i
		auxj=j
		while(auxi<=self.tamanio-1 and auxj<=self.tamanio-1):
			if(self.tablero[auxi][auxj]!='R'):
				self.tablero[auxi][auxj]='x'
			auxi+=1
			auxj+=1
		...
		while(auxi>=0 and auxj<=self.tamanio-1):
			if(self.tablero[i][j]!='R'):
				self.tablero[i][j]='x'
			auxi-=1
			auxj+=1
		auxi=i
		...
		while(auxi<=self.tamanio-1 and auxj>=0):
			if(self.tablero[auxi][auxj]!='R'):
				self.tablero[auxi][auxj]='x'
			auxi+=1
			auxj-=1
	def marcarVertical(self,j):
		for n in range(0,self.tamanio):
			if(self.tablero[n][j]!='R'):
				self.tablero[n][j]='x'
	def mostrar(self):
		for i in range(0,self.tamanio):
...
class interfaz():
	...
	def generarVentanaPrincipal(self):
		...
		self.frame.pack()
		Label(self.frame,text="PROBLEMA DE LAS N REINAS, PORFABOR SELECCIONE \n EL PRIMER TIRO PARA HACER EL ANALISIS POR AVARA",font=("comic sans ms",32)).grid(row=0,column=0,columnspan=15,pady=2)
		for i in range(0,8):
			self.botones.append([])
			for j in range(0,8):
		...
		Label(self.frame,text="SE GENERARAN TODAS LAS REINAS QUE NO SE ATACAN MUTUAMENTE",font=("comic sans ms",20)).grid(row=9,column=0,columnspan=15,padx=2,pady=2)
	...
	def generarMatriz(self):
		...
		reinas(matriz,self,self.iaux,self.jaux,self.primerTiro)
